...
const LIBRARY_FILE = path.join(OUTPUT_DIR, 'equipment-library.json');
...
async function fetchLibraryData() {
  console.log("🔄 Extrayendo datos de biblioteca y estadísticas objetivo...");

  // Estructura de la biblioteca de equipamiento
  const library = {
  ...
  // Base de datos de estadísticas sugeridas por personaje
  const targetStats = {
    genshin: {},
    hsr: {},
    zzz: {}
  ...
  try {
    // 1. Cargar biblioteca existente o inicializar
    if (fs.existsSync(LIBRARY_FILE)) {
      const existing = JSON.parse(fs.readFileSync(LIBRARY_FILE, 'utf-8'));
      Object.assign(library, existing);
    }

    // 2. Guardar archivos procesados
    ...
    console.log(`📁 Biblioteca guardada en: ${LIBRARY_FILE}`);
